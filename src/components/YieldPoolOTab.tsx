...
export default function YieldPoolTab({
  ...
  burn: (amountInBurnedYT : bigint) => void;
  loading?: boolean;
  RTSymbol: string;
}) {
  ...
  return (
    <div className="flex flex-col items-center text-white font-avenir">
      ...
      <div className="flex gap-x-2 mt-[0.81rem] ml-auto mr-10">
        ...
        <Link underline="always" className="text-[#B625FF]" onPress={() => setAmount(balance.toFixed(18) || "")}>
          MAX
        </Link>
      </div>
    <Button
        ...
        onClick={() => burn(BigInt(parseEther(amount || "")))}>
        Burn
    </Button>
    </div>
  );
